...
class PurchaseListViewSet(viewsets.ModelViewSet):
    ...
    def items(self, request, pk=None):
        ...
        if pl.status == "final":
            return Response({"detail": "No se pueden editar listas finalizadas."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def pdf(self, request, pk=None):
        ...
        if not pdf_bytes:
            return Response({"detail": "No se pudo generar el PDF en este entorno."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
